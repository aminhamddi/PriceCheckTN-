...
class ProductConsolidator:
    ...
    def load_latest_csv(self, source: str) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️  Loaded with latin-1 encoding")
                return df
    ...
    def standardize_columns(self, df: pd.DataFrame, source: str) -> pd.DataFrame:
        ...
        for col in required_columns:
            if col not in df_renamed.columns:
                logger.warning(f"⚠️  Missing column: {col} in {source}")
                df_renamed[col] = None
    ...
    def remove_duplicates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if duplicates_removed > 0:
            logger.info(f"🗑️  Removed {duplicates_removed} duplicates")
    ...
    def validate_products(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if invalid_removed > 0:
            logger.info(f"🗑️  Removed {invalid_removed} invalid products")
    ...
    def remove_duplicate_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if duplicate_cols:
            logger.warning(f"⚠️  Found duplicate columns: {duplicate_cols}")
    ...
    def consolidate_all(self) -> pd.DataFrame:
        ...
        for source in sources:
            ...
            if df.empty:
                logger.warning(f"⚠️  No data for {source}, skipping...")
                continue
