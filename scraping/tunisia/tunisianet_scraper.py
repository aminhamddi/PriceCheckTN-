...
from scraping.base_scraper import HybridScraper
...
class TunisianetScraper(HybridScraper):
    """
    ...
    def wait_for_content(self, page):
        ...
        try:
            page.wait_for_selector('article.product-miniature', timeout=15000)
        except:
            pass
    ...
    def build_page_url(self, category_url: str, page_num: int) -> str:
        ...
        return f"{self.base_url}{category_url}{separator}page={page_num}"
    ...
    def scrape_category(
        ...
        logger.info(f"  Scraping category: {category_key}")
    ...
    def print_summary(self, products: List[Dict]):
        ...
        if 'discount_percent' in df.columns:
            ...
            print(f"\n  Products on Sale: {len(discounted)}")
            if len(discounted) > 0:
