...
class MyTekHybridScraper:
    ...
    def __init__(self, config: dict):
        ...
        self.param_name = config['pagination']['param_name']
    ...
    def scrape_page_with_playwright(self, url: str) -> List[Dict]:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Find products (should have data-product-id now)
                items = soup.find_all('div', class_='product-container', attrs={'data-product-id': True})

                logger.info(f"Found {len(items)} products with data-product-id")
    ...
    def scrape_with_pagination(self, category_url: str, max_pages: int = None) -> List[Dict]:
        ...
        logger.warning("⚠️  Using Playwright (slow but necessary for JS)")
